...
该模块提供了用于并行化强化学习环境的核心工具类和方法,包括:
- 环境向量化包装器(同步和异步)
- 计时器和性能监控工具
- 观测空间处理工具
- 日志和可视化工具

这些工具使得能够同时运行多个环境实例,加速训练和评估过程。
"""
import inspect
...
def compute_episode_success_metrics(info, episode_has_crash=None, episode_out_of_road=None):
    """
    统一定义 episode 级成功指标。

    旧版 success_rate:
        只看是否到达终点 arrive_dest。

    新版 safe_success_rate:
        到达终点 + 无车辆/障碍物碰撞 + 未偏离道路。
    """
    arrived = bool(info.get("arrive_dest", False))
    has_crash = bool(info.get("crash_vehicle", False) or info.get("crash_object", False))
    out_of_road = bool(info.get("out_of_road", False))
    if episode_has_crash is not None:
        has_crash = bool(episode_has_crash)
    if episode_out_of_road is not None:
        out_of_road = bool(episode_out_of_road)
    safe_success = arrived and (not has_crash) and (not out_of_road)
    return {
        "arrived": arrived,
        "has_crash": has_crash,
...
def step_envs(
    *,
    ...
    total_episodes,
    device,
    episode_has_crash=None,
    episode_out_of_road=None,
):
    """
    执行向量化环境的一步交互,并处理奖励记录和终止状态
    
    Args:
        cpu_actions: CPU上的动作数组
        envs: 向量化环境对象
        episode_rewards: 每个episode的累积奖励
        episode_costs: 每个episode的累积成本
        result_recorder: 结果记录器字典
        total_steps: 总步数计数器
        total_episodes: 总episode数计数器
        device: 计算设备
        episode_has_crash: 每个环境当前episode是否出现过碰撞（车辆/障碍物）
        episode_out_of_road: 每个环境当前episode是否出现过偏离道路
        
    Returns:
        tuple: (观测, 奖励, 完成标志, 信息, masks, 总episodes, 总步数, episode奖励, episode成本)
    """
    # 执行环境步进
    ...
    for idx, d in enumerate(done):
        ...
        elif "velocity" in info[idx]:
            velocity = np.asarray(info[idx]["velocity"], dtype=np.float32)
            if velocity.ndim == 0:
                result_recorder["speed_km_h"].append(float(velocity) * 3.6)
            else:
                result_recorder["speed_km_h"].append(float(np.linalg.norm(velocity)) * 3.6)
        for key, value in info[idx].items():
            if key.startswith("resact_"):
                result_recorder[key].append(value)
        
        if episode_has_crash is not None:
            episode_has_crash[idx] = bool(
                episode_has_crash[idx] or info[idx].get("crash_vehicle", False) or info[idx].get("crash_object", False)
            )
        if episode_out_of_road is not None:
            episode_out_of_road[idx] = bool(episode_out_of_road[idx] or info[idx].get("out_of_road", False))
        ...
        if d:
            ...
            # 记录成功率：
            # success_rate 保持旧口径；safe_success_rate 为新的安全成功率。
            success_metrics = compute_episode_success_metrics(
                info[idx],
                episode_has_crash=None if episode_has_crash is None else episode_has_crash[idx],
                episode_out_of_road=None if episode_out_of_road is None else episode_out_of_road[idx],
            )
    ...
    # 计算masks(未完成的environment为1,完成的为0)
    ...
    if episode_has_crash is not None:
        episode_has_crash[:] = np.logical_and(episode_has_crash, masks.astype(bool))
    if episode_out_of_road is not None:
        episode_out_of_road[:] = np.logical_and(episode_out_of_road, masks.astype(bool))
    assert episode_rewards.shape == episode_rewards_old_shape
    
    # 更新总步数
    total_steps += obs[0].shape[0] if isinstance(obs, tuple) else obs.shape[0]
    
    # 将masks转换为tensor并移到指定设备
...
def make_envs(single_env_factory, num_envs=3, asynchronous=False):
    """
    将多个环境堆叠成向量化RL环境(同步或异步)
    
    Args:
        single_env_factory: 单个环境的工厂函数
        num_envs: 并行进程数量
        asynchronous: 是否使用异步模式
        
    Returns:
        VecEnv: 向量化环境对象
    """
    # 只有在num_envs>1时才使用异步模式
    ...
    # 根据模式选择环境包装器
    if asynchronous:
        envs = SubprocVecEnv(envs)  # 异步多进程环境
    else:
        assert num_envs == 1, "在一个进程中只能运行一个MetaDrive实例"
        envs = DummyVecEnv(envs)  # 同步单进程环境
