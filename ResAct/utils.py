...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, batch_size, device,image_size=84, pre_image_size=84,transform=None):
        ...
        self.prev_actions = np.empty((capacity, *action_shape), dtype=np.float32)
    ...
    def add(self,prev_obs, obs, action,prev_action, curr_reward, reward, next_obs, done):
        np.copyto(self.prev_obses[self.idx], prev_obs)
        np.copyto(self.obses[self.idx], obs)
        np.copyto(self.actions[self.idx], action)
        np.copyto(self.prev_actions[self.idx], prev_action)
    ...
    def sample(self,aug_funcs, k=False):
        ...
        if aug_funcs:
            for aug,func in aug_funcs.items():
                ...
                if 'crop' in aug or 'cutout' in aug:
                    ...
                    og_prev_obses = center_crop_images(prev_obses,self.pre_image_size)
                    ...
                    prev_obses = func(og_prev_obses, self.image_size, **rndm_idxs)  

        prev_obses = torch.as_tensor(prev_obses, device=self.device).float()
        obses = torch.as_tensor(obses, device=self.device).float()
        actions = torch.as_tensor(self.actions[idxs], device=self.device)
        prev_actions = torch.as_tensor(self.prev_actions[idxs], device=self.device)
        ...
        return prev_obses,obses, actions, prev_actions,curr_rewards, rewards, next_obses, not_dones
