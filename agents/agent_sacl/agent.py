...
class Policy:
    ...
    def __init__(
        ...
        default_state_shape = 259 + (2 if bool(cfg.get("resact_enabled", False)) else 0)
        state_shape = _infer_state_shape(model, default_state_shape)
        self.expected_observation_dim = int(state_shape)
        self.expected_base_observation_dim = int(state_shape - 2) if bool(cfg.get("resact_enabled", False)) else int(state_shape)
        action_shape = 2
        max_action = 1.0
        cost_dim = 1
