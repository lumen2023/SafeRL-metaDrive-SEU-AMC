...
def test_remove_crashed_traffic_vehicle():
    env = MetaDriveEnv(
        {
            "traffic_density": 1.0,
            ...
            "remove_crashed_traffic_vehicle": True,
        }
    )
    o, _ = env.reset()
    initial_traffic_num = len(env.engine.traffic_manager.traffic_vehicles)
    pass_test = False
    try:
        for i in range(1, 500):
            o, r, tm, tc, info = env.step([0, 1])
            if info["crash_vehicle"]:
                pass_test = True
                assert len(env.engine.traffic_manager.traffic_vehicles) < initial_traffic_num
                break
        assert pass_test, "Traffic vehicle removal on crash was not exercised!"
    finally:
        env.close()
