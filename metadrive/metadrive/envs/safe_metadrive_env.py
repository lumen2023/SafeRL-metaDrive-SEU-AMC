...
class SafeMetaDriveEnv(MetaDriveEnv):
    ...
    def reset(self, *args, **kwargs):
        ...
        # 修复：如果 kwargs 中包含 'options'，将其移除，因为父类 BaseEnv 不支持它
        if 'options' in kwargs:
            kwargs.pop('options')

        # 同样，如果有 'seed' 参数，确保处理方式正确（通常 MetaDrive 使用 force_seed）
        # 如果父类也不支持 seed，你可能也需要 pop 掉 seed，或者手动设置 self.seed(seed)
